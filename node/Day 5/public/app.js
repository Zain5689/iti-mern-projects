...
socket.on('task:updated', (updatedTask) => {
    ...
    if (index !== -1) {
        ...
        // If task not found, add it (shouldn't happen but just in case)
        tasks.push(updatedTask);
    }
    
    renderTasks();
    showToast(`Task "${updatedTask.title}" marked as ${updatedTask.done ? 'completed' : 'active'}`, 'info');
});
...
function toggleTaskCompletion(taskId, isChecked) {
    ...
    if (task) {
        ...
        socket.emit('task:toggle', { id: taskId });
        
        // Don't show toast here - it will be shown when server responds
